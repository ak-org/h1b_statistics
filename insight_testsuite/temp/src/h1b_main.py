...
def parse_input_params():
    '''
    Prase input parameters, values are returned as part of parser object
    '''
...
def main():
    ...
    main function performs following actions:
     ...
     - Invokes h1b process class
     - calculates total count of certified cases 
     - calculates number of unique states
     - calculates number of unique occupations
     - calculates cases for each state/occupation, calculates percentage 
     --  and saves the output in the file specified by the output parameters

    '''
    params = parse_input_params()
    (opt, _) = params.parse_args()
    ...
    if len(h1b_data) > 0:
        ...
        if __debug__:
            print("** States processing complete. **")
        ...
        if __debug__:
            print("** Occupation processing complete. **")
    
'''
use python -O option on command line to suppress the debug information
'''    
